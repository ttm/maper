...
$querystr = "SELECT post_id FROM $wpdb->postmeta WHERE $wpdb->postmeta.meta_key = '_mapasdeusuario' AND $wpdb->postmeta.meta_value = '$uid'";

//$mapas = $wpdb->get_results("SELECT post_id FROM $wpdb->postmeta");
...
foreach($mapas as $m){
    ...
    $foo=$m->{'post_id'};
    ?>
    <a href="?page_id=<?=$foo;?>">Mapa <?=$foo;?></a><br/>
    <?
}?>
